...
$aTabs = [
    [
        ...
        "OPTIONS" => [
            "Список параметров",
            ["JF_PATH_JSON", "Путь для сохранения json", null, ["text",50]],
        ]
    ],
    [
        "DIV" => "edit2",
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && $RIGHT=="W" && check_bitrix_sessid())
{
    ...
    if ($_REQUEST['JF_PATH_JSON'] != Option::get("json.forms", "JF_PATH_JSON")) {
        CheckDirPath($_SERVER["DOCUMENT_ROOT"].$_REQUEST['JF_PATH_JSON']);
    }
